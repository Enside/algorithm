...
void critical_path(LIST_P list)
{
    int *topo = topological_order(list);
    int etv[MAX] = {0};
    int ltv[MAX] = {0};
    EDGE_P temp;
    if (topo)
    {
        for (int i = 0; i < list->amount; i++)
        {
            temp = list->in_list[topo[i]];
            while (temp)
            {
                if (temp->weight + etv[temp->edge[0]] > etv[topo[i]])
                {
                    etv[topo[i]] = temp->weight + etv[temp->edge[0]];
                }
                temp = temp->in_next;
            }
        }
        ...
        {
            ltv[topo[i]] = etv[topo[list->amount - 1]];
            temp = list->out_list[topo[i]];
            while (temp)
            {
                if (ltv[temp->edge[1]] - temp->weight < ltv[topo[i]])
                {
                    ltv[topo[i]] = ltv[temp->edge[1]] - temp->weight;
                    if (ltv[topo[i]] < 0)
                    {
                        ltv[topo[i]]=0;
                    }
        ...
        for (int i = 0; i < list->amount; i++)
        {
            printf("%-3d", etv[topo[i]]);
        }
        putchar('\n');
        printf(" ltv:");
        for (int i = 0; i < list->amount; i++)
        {
            printf("%-3d", ltv[topo[i]]);
        }
        putchar('\n');
        printf("path:");
        for (int i = 0; i < list->amount; i++)
